...
def downscale_chunk(targetVar, methodName, family, mode, fields, scene, model, iproc=0, nproc=1):
    ...
    special_value = int(predictands_codification[targetVar]['special_value'])
    ...
    for ipoint in range(len(points_chunk[ichunk])):
        ...
        # Apply downscaling
        if methodName == 'QM':
            est[:, ipoint_local_index] = MOS_lib.quantile_mapping(Y_train, X_train, X_test, targetVar)[:, 0]
        if methodName == 'DQM':
...
if __name__=="__main__":
    ...
    fname = 'Calling down_scene_MOS __main__' + "\n"
    roxpath = "roxprint_output.txt"
    grasptools.roxprint(roxpath, fname)
